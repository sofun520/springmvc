...
<body class="hold-transition skin-blue sidebar-mini"
	...
	<div class="content">
									...
									<tbody>
										...
										<c:forEach items="${odList}" var="od" varStatus="xh">
											<tr>
												...
												<td align="right"><c:out value="${od.odNumber}" /></td>
												<td align="right"><c:out value="${od.pc.pcPrice}" /></td>
												<td align="right"><c:out
														value="${od.pc.pcPrice*od.odNumber}" /></td>
											</tr>
											<c:set var="sum" value="${sum+od.pc.pcPrice*od.odNumber}"></c:set>
										</c:forEach>
										<tr>
								...
								<table
									...
									<tbody>
										<c:forEach items="${kclist}" var="kc" varStatus="xh">
											<tr>
												<td><c:out value="${xh.count}" /></td>
												<td><c:out value="${kc.pName}" /></td>
												<td><c:out value="${kc.pXinghao}" /></td>
												<td><c:out value="${kc.pCount}" /></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
		...
		<div id="shenheBtn">
			<span style="font-size: 12px;">审核订单：</span> <a
				href="peihuo.do?orderId=${orderId}&status=1&id=${id}"
				data-dismiss="modal" aria-label="Close"
				class="btn btn-primary btn-sm">审核通过，下一步</a> &nbsp;&nbsp;
			<a
				href="peihuo.do?orderId=${orderId}&status=5&id=${id}"
				data-dismiss="modal" aria-label="Close"
				class="btn btn-default btn-sm">审核拒绝</a>
		</div>
	</div>
				</div>
